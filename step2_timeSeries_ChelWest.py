...
combined = pd.read_csv(path+'Data/'+'0_chelwest_combined.csv')
...
for i in hospital_site_list:
    for j in procedures_list[0:5]:
        
        #Hospital_site = 'CW'
        #Procedure_category = 'Colonoscopy'
        Hospital_site = i
        Procedure_category = j
        c = combined[(combined['Hospital site']==Hospital_site)&(combined['Procedure category']==Procedure_category)]
        c = c.sort_values(by=['Date'], ascending = [True])
        
        c.to_csv(path+'Results/0_ChelWest/'+str(n)+'_'+Hospital_site+'_'+Procedure_category+'_sliced_data.csv')
        
        # get and save the df with aggregated weekly counts
        d = get_table_with_weekly_counts(df, Hospital_site, Procedure_category)
        d.to_csv(path +'Results/0_ChelWest/'+str(n)+'_'+Hospital_site+'_'+Procedure_category+'_weekly_counts.csv')
        ...
        p.get_figure().savefig(path+'Results/0_ChelWest/'+str(n)+'_'+Hospital_site+'_'+Procedure_category+'.pdf')
        p.get_figure().savefig(path+'Results/0_ChelWest/'+str(n)+'_'+Hospital_site+'_'+Procedure_category+'.jpg', dpi=300)
